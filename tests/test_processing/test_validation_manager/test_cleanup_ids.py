        ...
        return MockResponse(
            {
                "results": {
                    "bindings": [
                        {
                            "protein": {
                                "value": "http://purl.uniprot.org/uniprot/Q8KND5",
                            },
                        },
                    ],
                },
        ...
        return MockResponse(
            {
                "results": {
                    "bindings": [
                        {
                            "protein": {
                                "value": "http://purl.uniprot.org/uniprot/Q8KND4",
                            },
                        },
                    ],
                },
...
def test_cleanup_ids_both_fail_1(mock_get):
    with pytest.raises(
        ValueError,
        match=re.escape(
            "The provided Genpept ID 'CAA71118.1' and Uniprot ID 'Q8KND5' do not correspond to each other!",
        ),
    ):
...
def test_cleanup_ids_both_fail_2(mock_get):
    with pytest.raises(
        ValueError,
        match=re.escape(
            "The provided Genpept ID 'AAM70353.1' and Uniprot ID 'Q8KND4' do not correspond to each other!",
        ),
    ):
...
def test_cleanup_ids_none(mock_get):
    with pytest.raises(
        ValueError,
        match=re.escape("Please provide one of NCBI Genpept or Uniprot IDs."),
    ):
...
def test_cleanup_ids_invalid(mock_get):
    with pytest.raises(
        ValueError,
        match=re.escape("HTTP Error while querying Uniprot: 404"),
    ):
